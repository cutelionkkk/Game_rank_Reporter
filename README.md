...
> ⚠️ **重要提示：第一次抓取不会有分析结果！**
>
> 排名变动分析（飙升、新上榜、暴跌、跌出榜单等）需要**至少两次抓取数据**进行对比。第一次运行只会保存当前排名快照，**从第二次运行开始**才会生成完整的变动分析报告。建议设置定时任务（每 12 小时一次），第二次运行后即可看到完整分析。
...
复制这句话给你的 AI Agent（Claude Code、OpenClaw、Cursor、Windsurf 等）：

```
帮我安装 Game Rank Reporter：https://raw.githubusercontent.com/cutelionkkk/Game_rank_Reporter/main/docs/install.md
```

就这一步。Agent 会自己完成：克隆仓库、安装依赖、运行首次抓取、配置通知渠道。

> 🤖 **兼容所有 Agent** — 任何能跑命令行的 AI Agent 都能用。Agent 读完 install.md 后知道该做什么。

<details>
<summary>Agent 会做什么？（点击展开）</summary>

1. `git clone` 克隆仓库
2. `pip install` 安装 Python 依赖
3. `npm install -g google-play-scraper` 安装 Node.js 依赖
4. 运行首次抓取验证安装
5. 交互式配置通知渠道
6. 可选：设置定时任务
...
除了自带的排名变动检测，Game Rank Reporter 还能导出结构化数据，供 AI Agent 做**深度市场分析**。
...
# 生成 analysis_data.json，包含所有榜单数据 + 变动信息
```

### AI 分析模板

项目内置了 3 个分析 prompt 模板（见 `docs/ai_analysis_guide.md`）：

| 模板 | 适用场景 | 分析维度 |
|------|---------|---------|
| **市场洞察** | 了解大盘趋势 | 品类分析、开发商分析、跨平台比较、市场机会 |
| **竞品追踪** | 监控特定竞品 | 竞品排名趋势、同品类竞争格局、变动原因推测 |
| **投资视角** | 投资决策参考 | 头部公司表现、品类生命周期、变现能力、风险信号 |

### 自动化 AI 分析

如果你的 Agent 支持定时任务，可以实现全自动流程：

```
抓取数据 → 导出 JSON → AI 读取分析 → 生成深度报告 → 推送到渠道
```

不只是"排名变了"，而是"排名变了，**意味着什么**"。

> 📖 完整的分析方法论、prompt 模板、数据结构说明见 [`docs/ai_analysis_guide.md`](docs/ai_analysis_guide.md)
